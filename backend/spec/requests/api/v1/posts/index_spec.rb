      ...
      context "with valid sorting parameter" do
        ...
        it "list posts and returns properly sorted resources" do
          ...
          expect(json_response["links"])
            .to eq(
              {
                "first" => "http://example.org/api/v1/posts?page%5Bnumber%5D=1&page%5Bsize%5D=2",
                "last" => "http://example.org/api/v1/posts?page%5Bnumber%5D=2&page%5Bsize%5D=2",
                "next" => "http://example.org/api/v1/posts?page%5Bnumber%5D=2&page%5Bsize%5D=2",
                "prev" => nil,
                "self" => "http://example.org/api/v1/posts?page%5Bnumber%5D=1&page%5Bsize%5D=2"
              }
            )
        end
      end
